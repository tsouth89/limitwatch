...
import { stripHtml, quoteInSource, parseFeed, parseHtml, MODEL_AVAILABILITY_GUIDANCE } from "./lib/discover-core.mjs";
...
if (process.env.ANTHROPIC_API_KEY && fresh.length) {
  ...
  const prompt = `You triage AI-provider news for a site that tracks CONSUMER SUBSCRIPTION usage limits, quotas, rate limits, model access, and plan pricing over time (e.g. Claude Pro/Max, ChatGPT Plus/Pro, Gemini AI Plus/Pro, Copilot Pro/Pro+, Cursor Pro). The bar is high: flag an item ONLY if a person on a NAMED consumer/prosumer plan or tracked subscriber surface would see a concrete change to what they can do or pay.\n\nINCLUDE only items that announce a concrete change to: a consumer plan's usage limit / quota / rate cap (messages, requests, tokens, credits per window), the price of a consumer plan, the credit pool bundled with a consumer plan, a new/removed consumer plan, or model availability for named subscriber plans/surfaces. ${MODEL_AVAILABILITY_GUIDANCE}\n\nEXCLUDE (do NOT flag) even if usage-adjacent: usage metrics, dashboards, analytics, reporting, or admin spend-control features (visibility ≠ a limit change); generic API/SDK tier or developer-platform changes; model releases or capability/context-window changes without a named subscriber access change; benchmark posts; security, infra, partnerships, and sales. When unsure, EXCLUDE.\n\nReturn a JSON array (no prose). Each element: {"index": <number>, "why": "<one sentence naming the consumer plan/surface and the concrete change>", "kind": "limit_boost|limit_cut|model_availability|pricing_change|promo|new_plan", "confidence": "official|announced"}. If none qualify, return [].\n\nItems:\n${list}`;
  try {
...
async function extractEvent(item, pageText, model) {
  const prompt = `You extract a structured subscription event from an AI provider's article, for a site that tracks subscription usage limits, model access, and consumer plan pricing over time.\n\nProvider: ${item.provider}${item.product ? " / " + item.product : ""}\nArticle title: ${item.title}\nArticle text (may be truncated):\n"""${pageText.slice(0, 7000)}"""\n\nReturn ONE JSON object, no prose:\n{\n  "is_limit_change": <true for a concrete subscription limit/price/credit change OR named-plan model access change>,\n  "certainty": "high|medium|low",\n  "quote": "<a SINGLE sentence copied EXACTLY, character-for-character, from the article text above that states the change. Do not paraphrase, fix, or shorten it. If no such sentence exists, set is_limit_change false.>",\n  "title": "<= 80 char headline. State ONLY what the article supports; never invent facts.",\n  "kind": "limit_boost|limit_cut|model_availability|pricing_change|promo|new_plan",\n  "model": "named model or null",\n  "models": ["named models"] or null,\n  "access": "included|limited|metered or null",\n  "applies_to": ["named subscriber plans/surfaces mentioned"],\n  "window": "5h|3h|day|week|month|null",\n  "factor": <number like 1.5 for +50%, 2 for 2x, or null>,\n  "starts_on": "YYYY-MM-DD or null",\n  "ends_on": "YYYY-MM-DD or null",\n  "permanent": <true|false>,\n  "confidence": "official|announced"\n}\nFor model_availability, require a named plan or tracked subscriber surface and set model or models plus access. Exclude generic API-only launches, benchmarks, and capability-only posts. The quote MUST be a verbatim substring of the article text. Be conservative: low certainty if unsure.`;
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: { "x-api-key": process.env.ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json" },
...
if (process.env.ANTHROPIC_API_KEY && relevant && relevant.length) {
  ...
  for (const r of relevant) {
    ...
    auto.events.unshift({
      ...
      ...(ev.kind === "model_availability" ? { model: ev.model || null, models: Array.isArray(ev.models) ? ev.models : null, access: ev.access || null } : {}),
